...
def encontrar_excel():
    arquivos = list(BASE_DIR.glob("*.xlsx")) + list(BASE_DIR.rglob("*.xlsx"))
    arquivos = [a for a in arquivos if not a.name.startswith("~$")]

    if not arquivos:
        st.error("Nenhum arquivo Excel .xlsx foi encontrado no projeto.")
        st.write("Pasta onde o app.py está rodando:")
        st.code(str(BASE_DIR))
        st.write("Arquivos encontrados:")
        st.write([str(x.relative_to(BASE_DIR)) for x in BASE_DIR.rglob("*") if x.is_file()])
        st.stop()

    for arquivo in arquivos:
        if "dashboard_financeiro" in arquivo.name.lower():
            return arquivo

    return arquivos[0]

ARQ_EXCEL = encontrar_excel()
...
/* CORREÇÃO DAS CAIXAS DE SELEÇÃO - texto visível */
...
st.sidebar.caption(f"Excel carregado: {ARQ_EXCEL.name}")
...
def velocimetro_ponteiro_grosso(percentual):
    valor = max(0, min(float(percentual), 120))
    theta = math.radians(180 - (valor / 120) * 180)
    x = 0.5 + 0.38 * math.cos(theta)
    y = 0.18 + 0.38 * math.sin(theta)
    ...
    fig.add_trace(go.Indicator(
        ...
        title={"text": "% da renda comprometida", "font": {"size": 22, "color": "#f8fafc"}},
        gauge={
            "axis": {"range": [0, 120], "tickwidth": 2, "tickcolor": "#f8fafc"},
            ...
            "steps": [
                ...
                {"range": [100, 120], "color": "#7f1d1d"},
            ],
        }
    ))

    # Ponteiro grosso e bem visível
    fig.add_shape(
        type="line",
        x0=0.5, y0=0.18, x1=x, y1=y,
        xref="paper", yref="paper",
        line=dict(color="#ffffff", width=14)
    )
    fig.add_shape(
        type="circle",
        x0=0.455, y0=0.135, x1=0.545, y1=0.225,
        xref="paper", yref="paper",
        fillcolor="#38bdf8",
